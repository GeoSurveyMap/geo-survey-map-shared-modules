...
var URLS = {
    ...
    updatePermissions: function (kindeId) {
        var params = new URLSearchParams({ kindeId: kindeId.toString() });
        return "/api/v1/user/update/".concat(params.toString());
    },
    ...
    banOrReactivate: function (kindeId, userStatus) {
        var params = new URLSearchParams({ kindeId: kindeId.toString() });
        return "/users/".concat(params.toString(), "/status/");
    }
};
...
function setUserPermissions(data) {
    ...
    return axiosClient_1.axiosClient.put(URLS.updatePermissions(kindeId), data);
}
